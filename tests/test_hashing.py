...
def test_deterministic():
    ...
    assert hash_dict(params) == hash_dict(params)
...
def test_float_meaningful_difference():
    ...
    h1 = hash_dict({"x": 1.0})
    h2 = hash_dict({"x": 1.001})
    assert h1 != h2


def test_enum_handling():
    """Enums are hashed by their value."""
    h1 = hash_dict({"color": Color.RED})
    h2 = hash_dict({"color": "red"})
    assert h1 == h2


def test_nested_dicts():
    """Nested dicts are normalized recursively."""
    h1 = hash_dict({"cfg": {"z": 1, "a": 2}})
    h2 = hash_dict({"cfg": {"a": 2, "z": 1}})
    assert h1 == h2
...
def test_bool_vs_int():
    ...
    h_bool = hash_dict({"flag": True})
    h_int = hash_dict({"flag": 1})
    assert h_bool != h_int
...
def test_tuple_treated_as_list():
    ...
    h_tuple = hash_dict({"x": (1, 2, 3)})
    h_list = hash_dict({"x": [1, 2, 3]})
